...
function setAssociationFields(field) {
  if (!associationFields.includes(field)) associationFields.push(field);
}
...
function isAssociationField(association) {
  for (i in association) {
    if (!isThisAssociationField(i)) return [false, i];
  }
  return [true, ''];
}
...
function compareAssociations(associationA, associationB) {
  ...
  return (
    associationA['name'].toLowerCase() === associationB['name'].toLowerCase() &&
    associationA['university'].toLowerCase() ===
      associationB['university'].toLowerCase()
  );
}
