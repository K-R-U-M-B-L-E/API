...
async function connectMongoDatabase(databaseName, collectionName) {
  try {
    ...
    console.error(
      '[DATABASE][database.js] Error while connecting to mongo database: ',
      e
    );
    console.error(
      '[DATABASE][database.js] Arguments: databaseName: ${databaseName} | collectionName: ${collectionName} ',
      e.stack
    );
  }
}
...
const ASSOCIATIONS_COLLECTION = {
  ...
  async getCollection() {
    if (this.connection == null) {
      if (process.env.TEST) {
        console.log(
          '[TEST][database.js]: ASSOCIATIONS_COLLECTION.getCollection()'
        );
        console.log('trying to connect to mongo database...');
        console.log('________________________________________________________');
      }
      this.connection = await connectMongoDatabase(
        DATABASE_NAME,
...
const UNIVERSITIES_COLLECTION = {
  ...
  async getCollection() {
    if (this.connection == null) {
      if (process.env.TEST) {
        console.log(
          '[TEST][database.js]: UNIVERSITES_COLLECTION.getCollection()'
        );
        console.log('trying to connect to mongo database...');
        console.log('________________________________________________________');
      }
